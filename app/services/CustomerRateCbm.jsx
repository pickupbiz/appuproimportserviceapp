...
const CustomerRateCbm = () => {
  ...
  const handleNext = () => {
    // Redux action example (commented)
    // const type = "BY_CBM";
    // const payload = { cbm, customerRate, ourRate, total: calculateCBM() };
  ...
  const calculateCBM = () => {
    ...
    const r = parseFloat(customerRate);
    if (isNaN(c) || isNaN(r)) return "";
    return (c * r).toFixed(2);
  };

  const isNextDisabled =
    !cbm || !customerRate || parseFloat(customerRate) <= parseFloat(ourRate);
    ...
    <View style={styles.container}>
      <View style={styles.card}>
        ...
        <Text style={styles.totalValue}>
          {calculateCBM() ? `Total: ${calculateCBM()}` : ""}
        </Text>
...
export default CustomerRateCbm;
